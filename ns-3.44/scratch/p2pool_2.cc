...
private:
    ...
    private:
        ...
        std::map<std::string, uint32_t> m_pendingBlockRequests;
    ...
    static Time FORWARD_INTERVAL;
    // For scheduled message sending with latency
    std::queue<std::pair<std::string, double>> m_messageQueue;
...
public:
    ...
    static void SetForwardInterval(uint32_t milliseconds) {
        FORWARD_INTERVAL = MilliSeconds(milliseconds);
    }
...
Time TcpGossipApp::FORWARD_INTERVAL = MilliSeconds(100);
...
private:
    ...
    // Pruning-specific members
    static constexpr uint32_t PRUNING_INTERVAL = 700;
    static constexpr uint32_t SAFETY_BLOCKS = 100; // Keep last 100 blocks for safety
    uint32_t m_lastPruningHeight;
    bool m_pruningEnabled;
    
    // Separate tracking for pruned vs active data
    std::set<std::string> m_prunedBlocks;
    uint32_t m_totalBlocksEverProcessed; // Track total including pruned
    uint32_t m_totalPrunedBlocks;
    
    // Keep minimal info for pruned blocks for analysis
    struct PrunedBlockInfo {
        uint32_t height;
        uint32_t nodeId;
        double timestamp;
        bool wasOnMainChain;
    ...
    std::map<std::string, PrunedBlockInfo> m_prunedBlocksInfo;
...
public:
    BlockchainState(uint32_t maxUnclesPerHeight = 5, TcpGossipApp* gossipApp = nullptr)
        : m_currentHead("genesis"), m_currentHeight(0),
          ...
          m_lastPruningHeight(0), m_pruningEnabled(true), 
          m_totalBlocksEverProcessed(0), m_totalPrunedBlocks(0) {
    ...
    void setPruningEnabled(bool enabled) { m_pruningEnabled = enabled; }
    bool isPruningEnabled() const { return m_pruningEnabled; }

    // Check if pruning is needed
    bool shouldPrune() const {
        return m_pruningEnabled && 
               (m_currentHeight - m_lastPruningHeight >= PRUNING_INTERVAL);
    }
    ...
    void performPruning() {
        ...
        uint32_t nodeId = m_gossipApp ? m_gossipApp->GetNodeId() : 0;
        NS_LOG_INFO("Node " << nodeId << " starting pruning at height " << m_currentHeight);

        uint32_t pruneUpToHeight = m_currentHeight - SAFETY_BLOCKS;
        std::vector<std::string> blocksToPrune;

        // Collect blocks to prune (keep safety blocks)
        for (const auto& pair : m_blockchain) {
            if (pair.first != "genesis" && pair.second->blockNumber <= pruneUpToHeight) {
                blocksToPrune.push_back(pair.first);
            }
        }

        // Store minimal info before pruning for analysis
        for (const std::string& blockHash : blocksToPrune) {
            auto blockIt = m_blockchain.find(blockHash);
            if (blockIt != m_blockchain.end()) {
                PrunedBlockInfo info;
                info.height = blockIt->second->blockNumber;
                info.nodeId = blockIt->second->nodeId;
                info.timestamp = blockIt->second->timestamp;
                info.wasOnMainChain = isOnMainChain(blockHash);
                info.wasUncle = (m_orphanBlocks.find(blockHash) == m_orphanBlocks.end()) && !info.wasOnMainChain;
                info.wasOrphan = (m_orphanBlocks.find(blockHash) != m_orphanBlocks.end());

                m_prunedBlocksInfo[blockHash] = info;
                m_prunedBlocks.insert(blockHash);
            }
        }

        // Remove from main data structures but keep DAG structure
        for (const std::string& blockHash : blocksToPrune) {
            // Remove from blockchain map
            m_blockchain.erase(blockHash);
            
            // Remove from height-based indexing
            for (auto& heightPair : m_blocksByHeight) {
                auto& blocksAtHeight = heightPair.second;
                blocksAtHeight.erase(std::remove(blocksAtHeight.begin(), blocksAtHeight.end(), blockHash), blocksAtHeight.end());
            }
            
            // Clean up empty height entries
            for (auto it = m_blocksByHeight.begin(); it != m_blocksByHeight.end();) {
                if (it->second.empty() && it->first <= pruneUpToHeight) {
            ...
            // Remove from uncle tracking
            for (auto& uncleHeightPair : m_unclesByHeight) {
                auto& unclesAtHeight = uncleHeightPair.second;
                unclesAtHeight.erase(std::remove(unclesAtHeight.begin(), unclesAtHeight.end(), blockHash), unclesAtHeight.end());
            }

            // Remove from orphan tracking
            m_orphanBlocks.erase(blockHash);

            // NOTE: We keep the DAG structure intact for visualization
            // The vertices remain but without full block data
        }

        m_totalPrunedBlocks += blocksToPrune.size();
        m_lastPruningHeight = m_currentHeight;

        NS_LOG_INFO("Node " << nodeId << " pruned " << blocksToPrune.size() 
                   << " blocks up to height " << pruneUpToHeight 
                   << ". Total pruned: " << m_totalPrunedBlocks);
    }
    ...
    ChainAnalysis getDetailedAnalysis() const {
        ...
        // Always use simple, correct calculation
        analysis.totalBlocks = (m_blockchain.size() - 1) + m_totalPrunedBlocks; // Exclude genesis
        ...
        // Count uncles (active + pruned)
        uint32_t activeUncles = 0;
        for (const auto& pair : m_unclesByHeight) {
            activeUncles += pair.second.size();
        }
        
        uint32_t prunedUncles = 0;
        for (const auto& pair : m_prunedBlocksInfo) {
            if (pair.second.wasUncle) prunedUncles++;
        }
        
        analysis.uncleBlocks = activeUncles + prunedUncles;
        
        // Miner distribution (active + pruned)
        for (const auto& pair : m_blockchain) {
            if (pair.first != "genesis") {
                analysis.minerDistribution[pair.second->nodeId]++;
            }
        }
        
        // Add pruned blocks to miner distribution
        for (const auto& pair : m_prunedBlocksInfo) {
            analysis.minerDistribution[pair.second.nodeId]++;
        }
        ...
        // Calculate average block time (active blocks only for accuracy)
        double totalTime = 0;
        uint32_t timeBlocks = 0;
        for (const auto& pair : m_blockchain) {
            if (pair.first != "genesis" && pair.second->timestamp > 0) {
                totalTime += pair.second->timestamp;
                timeBlocks++;
            }
        }
    ...
    ForkInfo getForkAnalysis() const {
        ...
        uint32_t totalBlocks = (m_blockchain.size() - 1) + m_totalPrunedBlocks; // Include pruned blocks
        uint32_t mainChainBlocks = m_currentHeight; // Blocks on main chain (excluding genesis)
        uint32_t sideBlocks = 0;

        // Count blocks not on main chain (active blocks)
        for (const auto& pair : m_blockchain) {
        ...
        // Add pruned side blocks
        for (const auto& pair : m_prunedBlocksInfo) {
            if (!pair.second.wasOnMainChain) {
                sideBlocks++;
            }
        }
        ...
        for (const auto& heightPair : m_blocksByHeight) {
            ...
            if (height > 0 && blocksAtHeight.size() > 1) {
                ...
                std::string parentHash = "";
                bool foundParent = false;
                std::vector<std::string> forkedBlocks;
                
                // Find common parent of blocks at this height and collect all blocks
                for (const auto& blockHash : blocksAtHeight) {
                    ...
                    if (blockIt != m_blockchain.end()) {
                        ...
                        forkedBlocks.push_back(blockHash);
                    }
                }
                
                // Add all blocks at this height to the fork branches
                if (foundParent && forkedBlocks.size() > 1) {
                    for (const auto& forkBlock : forkedBlocks) {
                        forkInfo.forkBranches[parentHash].push_back(forkBlock);
                    }
                    
                    // Add parent to fork points if not already added
                    if (std::find(forkInfo.forkPoints.begin(), forkInfo.forkPoints.end(), parentHash) == forkInfo.forkPoints.end()) {
                        forkInfo.forkPoints.push_back(parentHash);
                    }
                    
                    uint32_t forkDepth = forkedBlocks.size() - 1;
        ...
        // Correct fork ratio calculation (including pruned blocks)
    ...
    void exportDAGToDot(const std::string& filename, uint32_t nodeId = 0) const {
        ...
        // Add title with pruning info
        dotFile << "  labelloc=\"t\";\n";
        dotFile << "  label=\"Blockchain DAG - Node " << nodeId << "\\n";
        dotFile << "Height: " << m_currentHeight << ", Head: " << getShortHash(m_currentHead) << "\\n";
        if (m_pruningEnabled) {
            dotFile << "Pruned: " << m_totalPrunedBlocks << " blocks, Safety: " << SAFETY_BLOCKS << "\";\n";
        } else {
            dotFile << "Pruning: DISABLED\";\n";
        }
        dotFile << "\n";
        ...
        std::map<std::string, std::string> colors = {
            ...
            {"orphan", "lightcoral"},
            {"missing", "lightgray"},
            {"pruned_mainchain", "darkgreen"},
            {"pruned_uncle", "orange"},
            {"pruned_orphan", "red"}
        };

        // Export vertices (both active and pruned blocks shown differently)
        auto vertexRange = boost::vertices(m_dag);
        for (auto it = vertexRange.first; it != vertexRange.second; ++it) {
            ...
            if (blockHash == "genesis") {
                ...
                label = "Genesis\\n(0)";
            } else if (m_prunedBlocks.find(blockHash) != m_prunedBlocks.end()) {
                // This is a pruned block - show with different styling
                auto prunedInfo = m_prunedBlocksInfo.find(blockHash);
                if (prunedInfo != m_prunedBlocksInfo.end()) {
                    if (prunedInfo->second.wasOnMainChain) {
                        color = colors["pruned_mainchain"];
                        label = shortHash + "\\nH:" + std::to_string(prunedInfo->second.height) +
                               "\\nM:" + std::to_string(prunedInfo->second.nodeId) + "\\n(PRUNED-MAIN)";
                    } else if (prunedInfo->second.wasUncle) {
                        color = colors["pruned_uncle"];
                        label = shortHash + "\\nH:" + std::to_string(prunedInfo->second.height) +
                               "\\nM:" + std::to_string(prunedInfo->second.nodeId) + "\\n(PRUNED-UNCLE)";
                    } else {
                        color = colors["pruned_orphan"];
                        label = shortHash + "\\nH:" + std::to_string(prunedInfo->second.height) +
                               "\\nM:" + std::to_string(prunedInfo->second.nodeId) + "\\n(PRUNED-ORPHAN)";
                    }
                } else {
            ...
            // Add different styling for pruned blocks
            if (m_prunedBlocks.find(blockHash) != m_prunedBlocks.end()) {
        ...
        // Enhanced legend with pruning info
        dotFile << "\n  // Legend\n";
        dotFile << "  subgraph cluster_legend {\n";
        dotFile << "    label=\"Legend\";\n";
        dotFile << "    style=filled; fillcolor=white;\n";
        ...
        if (m_totalPrunedBlocks > 0) {
            dotFile << "    legend_pruned_main [label=\"Pruned Main\", fillcolor=\"" << colors["pruned_mainchain"] << "\", style=\"filled,dashed\", shape=box];\n";
            dotFile << "    legend_pruned_uncle [label=\"Pruned Uncle\", fillcolor=\"" << colors["pruned_uncle"] << "\", style=\"filled,dashed\", shape=box];\n";
            dotFile << "    legend_pruned_orphan [label=\"Pruned Orphan\", fillcolor=\"" << colors["pruned_orphan"] << "\", style=\"filled,dashed\", shape=box];\n";
        }
        dotFile << "  }\n";

        // Enhanced statistics with pruning info
        dotFile << "\n  // Statistics\n";
        dotFile << "  subgraph cluster_stats {\n";
        dotFile << "    label=\"Statistics\";\n";
        dotFile << "    style=filled; fillcolor=lightyellow;\n";
        
        uint32_t activeBlocks = boost::num_vertices(m_dag) - 1 - m_totalPrunedBlocks;
        uint32_t totalEverProcessed = activeBlocks + m_totalPrunedBlocks;
        
        dotFile << "    stats [shape=plaintext, label=<\n";
        dotFile << "    <TABLE BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\">\n";
        dotFile << "    <TR><TD>Metric</TD><TD>Value</TD></TR>\n";
        dotFile << "    <TR><TD>Active Blocks</TD><TD>" << activeBlocks << "</TD></TR>\n";
        dotFile << "    <TR><TD>Pruned Blocks</TD><TD>" << m_totalPrunedBlocks << "</TD></TR>\n";
        dotFile << "    <TR><TD>Total Processed</TD><TD>" << totalEverProcessed << "</TD></TR>\n";
        dotFile << "    <TR><TD>Current Height</TD><TD>" << m_currentHeight << "</TD></TR>\n";
        dotFile << "    <TR><TD>Safety Buffer</TD><TD>" << SAFETY_BLOCKS << "</TD></TR>\n";
        dotFile << "    </TABLE>\n";
        dotFile << "    >];\n";
        dotFile << "  }\n";
        ...
        NS_LOG_INFO("Enhanced DAG with pruning info exported to: " << filename);
    }
    ...
    void printDAGState(uint32_t nodeId) const {
        ...
        NS_LOG_INFO("Orphan/uncles blocks in DAG: " << orphanCount);
    ...
    // Getter methods for pruning statistics
    uint32_t getTotalPrunedBlocks() const { return m_totalPrunedBlocks; }
    uint32_t getTotalBlocksProcessed() const { return m_totalBlocksEverProcessed; }
    uint32_t getLastPruningHeight() const { return m_lastPruningHeight; }
    uint32_t getSafetyBlocks() const { return SAFETY_BLOCKS; }
    bool isPruned(const std::string& blockHash) const {
        return m_prunedBlocks.find(blockHash) != m_prunedBlocks.end();
    }
};
...
private:
    ...
    // DAG-based blockchain state management with pruning support
...
public:
    MinerApp() : m_difficulty(1), m_lastReorgTime(0.0), m_reorgCount(0), m_miningRestarts(0),
        ...
        // Initialize DAG-based blockchain state with pruning enabled
        m_blockchain = std::make_unique<BlockchainState>(2); // Max 5 uncles per height
    ...
    virtual void StopApplication() override {
        ...
        uint32_t nodeId = GetNode()->GetId();
        NS_LOG_INFO("=== FINAL STATISTICS FOR NODE " << nodeId << " ===");
        ...
        NS_LOG_INFO("Total blocks processed: " << m_blockchain->getTotalBlocksProcessed());
        NS_LOG_INFO("Total blocks pruned: " << m_blockchain->getTotalPrunedBlocks());
        NS_LOG_INFO("Last pruning height: " << m_blockchain->getLastPruningHeight());
    ...
    // node statistics including pruning info
    std::map<std::string, double> getNodeStats() const {
        ...
        stats["total_pruned"] = static_cast<double>(m_blockchain->getTotalPrunedBlocks());
        stats["total_processed"] = static_cast<double>(m_blockchain->getTotalBlocksProcessed());
        stats["last_pruning_height"] = static_cast<double>(m_blockchain->getLastPruningHeight());
        stats["safety_blocks"] = static_cast<double>(m_blockchain->getSafetyBlocks());
    ...
    void SetSimulationStopTime(double stopTime) {
        m_stopMiningTime = stopTime - 10.0;
    }
    ...
    // Method to manually trigger pruning (for testing)
    void triggerPruning() {
        if (m_blockchain->shouldPrune()) {
            m_blockchain->performPruning();
        }
    }

    // Get pruning statistics
    std::map<std::string, uint32_t> getPruningStats() const {
        return {
            {"total_pruned", m_blockchain->getTotalPrunedBlocks()},
            {"total_processed", m_blockchain->getTotalBlocksProcessed()},
            {"last_pruning_height", m_blockchain->getLastPruningHeight()},
            {"current_height", m_blockchain->getCurrentHeight()},
            {"safety_blocks", m_blockchain->getSafetyBlocks()}
        };
    }
...
private:
    ...
    bool AttemptMining() {
        // uint32_t totalNodes = 100;  //  make this configurable for the meeet 
        double successProbability = 1.0 / s_totalNodes;
    ...
    void MineBlock() {
        ...
        if (!AttemptMining()) {
            // Mining failed - schedule next attempt
            ScheduleNextMining();
            return;
        }
        ...
        if (added) {
            ...
            // Check if pruning was triggered
            if (m_blockchain->getTotalPrunedBlocks() > 0) {
                auto pruningStats = getPruningStats();
                NS_LOG_INFO("Node " << nodeId << " pruning status: " 
                           << pruningStats["total_pruned"] << " blocks pruned, "
                           << "last at height " << pruningStats["last_pruning_height"]);
            }
...
// double MinerApp::s_meanMiningInterval = 30.0; // 30 seconds mean
...
uint32_t MinerApp::s_totalNodes = 100;// default total nodes in the network

double MinerApp::s_meanMiningInterval = 12.0;   // 12 seconds mean
double MinerApp::s_stdMiningInterval = 2.0;     // 2 seconds variation  
double MinerApp::s_minMiningInterval = 8.0;     // 8 seconds minimum
double MinerApp::s_maxMiningInterval = 20.0;    // 20 seconds maximum
...
class NetworkMonitor {
   ...
   void AnalyzeNetworkConsensus(const std::vector<Ptr<Node>>& nodes, uint32_t numNodes) {
    NS_LOG_INFO("################################################################################");
    NS_LOG_INFO(" DAG BLOCKCHAIN NETWORK ANALYSIS (WITH PRUNING)");
    NS_LOG_INFO("################################################################################");

    std::map<std::string, std::vector<uint32_t>> consensusGroups;
    std::map<uint32_t, std::vector<uint32_t>> heightGroups;
    uint32_t totalPrunedBlocks = 0;
    uint32_t totalProcessedBlocks = 0;

    // Collect data from all nodes including pruning stats
    for (uint32_t i = 0; i < numNodes; ++i) {
        ...
        if (node->GetNApplications() > 1) {
            ...
            if (minerApp) {
                ...
                if (blockchain) {
                    ...
                    totalPrunedBlocks += blockchain->getTotalPrunedBlocks();
                    totalProcessedBlocks += blockchain->getTotalBlocksProcessed();
                }
            }
        }
    }

    // Enhanced consensus analysis with pruning info
    NS_LOG_INFO("\n=== CONSENSUS ANALYSIS ===");
    NS_LOG_INFO("Number of different chain heads: " << consensusGroups.size());
    NS_LOG_INFO("Total blocks pruned across network: " << totalPrunedBlocks);
    NS_LOG_INFO("Total blocks processed across network: " << totalProcessedBlocks);
    ...
    if (totalProcessedBlocks > 0) {
        double pruningRatio = (100.0 * totalPrunedBlocks) / totalProcessedBlocks;
        NS_LOG_INFO("\n=== PRUNING EFFICIENCY ===");
        NS_LOG_INFO("Network pruning ratio: " << std::fixed << std::setprecision(1) 
                   << pruningRatio << "% of blocks pruned");
        
        if (pruningRatio > 70) {
            NS_LOG_INFO("✓ PRUNING STATUS: EFFICIENT");
        } else if (pruningRatio > 40) {
            NS_LOG_INFO("⚠ PRUNING STATUS: MODERATE");
        } else {
            NS_LOG_INFO("ℹ PRUNING STATUS: LOW (early simulation or disabled)");
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    // Mining parameters
    double meanMiningInterval = 12.0;    // 12 seconds mean
    double stdMiningInterval = 2.0;      // 2 seconds variation  
    double minMiningInterval = 8.0;      // 8 seconds minimum
    double maxMiningInterval = 20.0;     // 20 seconds maximum
    
    // Network parameters
    double maxNetworkDelay = 2.0;        // Up to 2 seconds network delay
    double miningVariation = 1.0;        // ±1 second mining variation
    
    // Node and gossip parameters
    uint32_t totalNodes = 100;           // Total nodes for mining probability
    uint32_t forwardInterval = 100;      // Forward interval in milliseconds
    uint32_t maxUncles = 2;             // Maximum uncles per height
    ...
    BlockchainState blockchain(2);  // Max 2 uncles per height
    ...
    MinerApp::SetMiningParameters(meanMiningInterval, stdMiningInterval, minMiningInterval, maxMiningInterval);
    
    // Set network parameters  
    MinerApp::SetNetworkParameters(maxNetworkDelay, miningVariation);
    
    // Set total nodes for mining probability
    MinerApp::SetTotalNodes(totalNodes);
    
    // Set forward interval
    TcpGossipApp::SetForwardInterval(forwardInterval);
    ...
    for (uint32_t i = 0; i < numNodes; i++) {
        ...
        // Configure pruning
        const BlockchainState* blockchain = minerApp->GetBlockchainState();
        const_cast<BlockchainState*>(blockchain)->setPruningEnabled(enablePruning);
   ...
   // Create the network monitor
    ...
    std::cout << " Pruning enabled: " << (enablePruning ? "YES" : "NO") << std::endl;
    std::cout << " Pruning interval: 700 blocks" << std::endl;
    std::cout << " Safety buffer: 100 blocks" << std::endl;
